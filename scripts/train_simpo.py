...
from utils import dpo_loss, linear_warmup_schedule, simpo_loss
from torch.optim.lr_scheduler import LambdaLR
import argparse

run_name = datetime.now().strftime("%Y%m%d-%H%M%S")

writer = SummaryWriter(log_dir=os.path.join("../output", "simpo", run_name))

def evaluate(model, dataloader, device, global_step, beta=0.01, gamma=1.0):
    model.eval()
    total_loss = 0
    count = 0
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            loss, _ = simpo_loss(model, batch, beta=beta, gamma=gamma)
...
def finetune_model():
    checkpoint_dir = f"../checkpoints/simpo_{args.save_name}"
    ...
    for epoch in range(num_epochs):
        ...
        for batch in pbar:
            ...
            if global_step % 1000 == 0:
                eval_loss = evaluate(model, eval_dataloader, device, global_step, args.beta, args.gamma)
...
def parse_args():
    ...
    parser.add_argument("--beta", type=float, default=2.5, help="Beta parameter for DPO loss.")
    parser.add_argument("--gradient_accumulation_steps", type=int, default=8, help="Number of gradient accumulation steps.")
    parser.add_argument("--gamma", type=float, default=1.0, help="Gamma parameter for DPO loss.")
